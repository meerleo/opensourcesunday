---
title: Sermon on the Mount
linkTitle: Sermon on the Mount
weight: 1
description: A nine-week middle school series through the Sermon on the Mount (Matthew 5–7).
cascade:
  params:
    reversePagination: false
---

**A nine-week series through the Sermon on the Mount.**
**Scripture anchor:** Matthew 5–7
**Series ONE thing:** Jesus doesn't just want us to follow the rules harder — he's inviting us into a brand new way to be human, built on him.

## Jump to a lesson

{{< cards >}}
  {{< card link="1-up-the-mountain" title="Lesson 1 · Up the Mountain" subtitle="Matthew 5:1–12 · The Beatitudes — who gets called blessed?" icon="map" >}}
  {{< card link="2-salt-and-light" title="Lesson 2 · Salt and Light" subtitle="Matthew 5:13–16 · Flavor and illumination" icon="light-bulb" >}}
  {{< card link="3-but-i-say" title="Lesson 3 · But I Say" subtitle="Matthew 5:17–48 · Heart, not just behavior" icon="heart" >}}
  {{< card link="4-when-you-pray" title="Lesson 4 · When You Pray" subtitle="Matthew 6:5–15 · Talk to your Father" icon="chat-alt" tag="draft" tagColor="yellow" >}}
  {{< card link="5-treasures-and-two-masters" title="Lesson 5 · Treasures & Two Masters" subtitle="Matthew 6:19–24 · What has your heart has your life" icon="sparkles" tag="draft" tagColor="yellow" >}}
  {{< card link="6-do-not-worry" title="Lesson 6 · Do Not Worry" subtitle="Matthew 6:25–34 · Trust the One who feeds sparrows" icon="cloud" tag="draft" tagColor="yellow" >}}
  {{< card link="7-judge-not" title="Lesson 7 · Judge Not" subtitle="Matthew 7:1–6 · Check the plank before the speck" icon="eye" tag="draft" tagColor="yellow" >}}
  {{< card link="8-ask-seek-knock" title="Lesson 8 · Ask, Seek, Knock" subtitle="Matthew 7:7–12 · A good Father and the Golden Rule" icon="key" tag="draft" tagColor="yellow" >}}
  {{< card link="9-house-on-the-rock" title="Lesson 9 · House on the Rock" subtitle="Matthew 7:24–29 · Hear and do. Build on Jesus." icon="home" tag="draft" tagColor="yellow" >}}
{{< /cards >}}

{{< callout type="info" >}}
**Leading this series?** The nine lessons are designed as one arc — identity first, then witness, heart, prayer, treasure, trust, judgment, asking, and finally foundation. Lesson 9 builds an explicit series recap into the teaching, so the ordering matters. If you only have time for a shorter arc, at minimum do Lessons 1 and 9 together.
{{< /callout >}}
